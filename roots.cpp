...
extern int quadratic_solve(double a, double b, double c, double & x1, double & x2 );


int root_interv_list(int deg, const double* poly_coefs, double b_bound, double sign, double *alist, double *blist, int & root_list_len);

int core_alg_01(int deg, const double* poly_coefs, double* palist, double *pblist,
							double* & palist_next, double* & pblist_next, int depth);


double binom[POLY_MAX_DEG + 1][POLY_MAX_DEG + 1];

double coefs_save[POLY_MAX_DEG + 1];

void print_coefs(int deg, double *poly_coefs)
{
	for(int i = 0; i <= deg; ++i) {
		cout << poly_coefs[i] << " x^" << (deg- i) << " + ";
	}
	cout << endl;
}
...
int make_trafo_1(int deg, double* coefs_in)
{
	DEBUG("start_trafo1");
...
int make_trafo_2(int deg, double* coefs_in)
{
	double mult = 1;
	int i = 0;
...
int make_trafo_3(int deg, double* coefs_in)
{
	int i = deg - 1;
	int j;
...
int make_trafo_divide(int deg, double * coefs_in)
{
...
double eval(int deg, const double *coefs, double x)
{
	double sum = 0;
	for(int i = 0; i <= deg; ++i) {
		sum = sum * x + coefs[i];
...
// compute p(x/b)
int poly_scale(int deg, double* coefs_in, double b)
{
	double mult = 1;
	for(int i = deg; i >= 0; --i) {
		coefs_in[i] *= mult;
...
int sign_variations(int deg, double* coefs)
{
...
int bisect(int deg, const double* poly_coefs, double x0, double x1, double & x_root)
{
	double y0, y1;
	double ymid;
	double xmid = 0.5*(x1 + x0);
	const double eps = 1e-10; // was 1e-6;
	const double eps_xdist = 1e-10; // was 1e-6

	//cout << "bisect:" << x0 << " " << x1 << endl;

	y0 = eval(deg, poly_coefs, x0);
	y1 = eval(deg, poly_coefs, x1);
	...
	while (fabs(x1 - x0) > eps_xdist) {
		//cout << "x0 = " << x0 << " x1 = " << x1 << endl;
		xmid = 0.5*(x0 + x1);
		ymid = eval(deg, poly_coefs, xmid);
		if (fabs(ymid) < eps) {
...
int find_root_list(int deg, const double* poly_1, double *alist, double* blist, int n_len, double* x0_list,
		double * & x0_list_next)
{
	int j = 0;
	double x0 = -DBL_MAX;
	while (j < n_len) {
		bisect(deg, poly_1, alist[j], blist[j], x0);
		*x0_list = x0;
		++x0_list;
		++j;
	}
...
int sort_list(int len, double* list)
{
...
int root_final_list(int deg, const double* poly_coefs, double b_bound, double * x0_list, int & x0_len)
{
	...
	root_interv_list(deg, poly_1, b_bound, +1, alist, blist, ablist_len);

	//cout << "ablist_len = " << ablist_len << endl;

	if (ablist_len > 0) {
		int ret = find_root_list(deg, poly_coefs, alist, blist, ablist_len, x0_list, x0_list_next);
	...
	poly_scale(deg, poly_1, -1);
	root_interv_list(deg, poly_1, 200, -1, alist, blist, ablist_len);
	...
	// sorts into ascending order
	sort_list(x0_len, x0_list_0);

#ifdef DEBUG_IT

	if (x0_len == 0 && deg == 2) {
		double a = coefs_save[0];
		double b = coefs_save[1];
		double c = coefs_save[2];
...
int root_interv_list(int deg, const double* poly_coefs, double b_bound, double sign,
		double *alist, double *blist, int & root_list_len)
{
	...
	poly_scale(deg, poly_1, b_bound);

	DEBUG( "poly scaled");

	KEYWAIT;

	core_alg_01(deg, poly_1, alist, blist, palist_next, pblist_next, 0);
		...
		double* & palist_next, double* & pblist_next, int depth)
{
	if (depth > 20) {
		...
		pblist_next = pblist;
#if 0
		cout << "core_alg_01..." << endl;
		cout << "deg = " << deg << endl;
		double poly_x[POLY_MAX_DEG+1];
		memcpy(poly_x, poly_coefs, sizeof(poly_x));
		print_coefs(deg, poly_x);
#endif
		return 0;
	}
	...
	make_trafo_1(deg, poly_1);

	DEBUG("trafo 1 done");

	int vars = sign_variations(deg, poly_1);
    if (vars == 0) {
    	palist_next = palist;
    	pblist_next = pblist;
    ...
    double midval = eval(deg, poly_coefs, 0.5);
    ...
    make_trafo_2(deg, poly_1);

    memcpy(poly_2, poly_1, copy_size);

    DEBUG("trafo2 done");

    core_alg_01(deg, poly_1, palist, pblist, palist_next1, pblist_next1, depth + 1);
    ...
    make_trafo_3(deg, poly_2);

    DEBUG("trafo3 done");

    core_alg_01(deg, poly_2, palist, pblist, palist_next1, pblist_next1, depth + 1);
...
void print_intervals(int len, double *alist, double *blist) {
	for(int i = 0; i < len; ++i){
		cout << "[ " << alist[i] << " , " << blist[i] << " ]" << endl;
	}
}

void list_out(int len, double* list)
{
	cout << "( ";
	for(int i = 0; i < len; ++i) {
		cout << list[i] << " , ";
